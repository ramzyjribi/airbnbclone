...
export class BookingService {
  ...
  create(newBooking: CreateBooking) {
    this.http.post<boolean>(`${environment.API_URL}/booking/create`, newBooking,{ withCredentials: true })
      .subscribe({
  ...
  checkAvailability(publicId: string): void {
    ...
    this.http.get<Array<BookedDatesDTOFromServer>>(`${environment.API_URL}/booking/check-availability`, {params,withCredentials: true })
      .pipe(
  ...
  getBookedListing(): void {
    this.http.get<Array<BookedListing>>(`${environment.API_URL}/booking/get-booked-listing`,{ withCredentials: true })
      .subscribe({
        next: bookedListings =>
  ...
  cancel(bookingPublicId: string, listingPublicId: string, byLandlord: boolean): void {
    ...
    this.http.delete<string>(`${environment.API_URL}/booking/cancel`, {params, withCredentials: true})
      .subscribe({
  ...
  getBookedListingForLandlord(): void {
    this.http.get<Array<BookedListing>>(`${environment.API_URL}/booking/get-booked-listing-for-landlord`,{ withCredentials: true })
      .subscribe({
        next: bookedListings =>
